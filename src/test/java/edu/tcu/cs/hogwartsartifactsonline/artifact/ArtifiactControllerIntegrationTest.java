...
@Tag("Integration")
public class ArtifiactControllerIntegrationTest {
    ...
    void testAddArtifactSuccess() throws Exception{
        ...
        a.setDescription("A Remembrall was a magical large marble-sized glass ball that contained smoke");
        a.setImageUrl("ImageUrl");

        String json = this.objectMapper.writeValueAsString(a);
        this.mockMvc.perform(MockMvcRequestBuilders.get(this.baseUrl+ "/artifacts").contentType(MediaType.APPLICATION_JSON).header("Authorization", this.token).content(json).accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
